...
func musicXMLRealisticPlaybackDefaultsAreHardcodedForNoSettingsSwitches() {
    ...
    #expect(profile.breathGapTicks == MusicXMLTempoMap.ticksPerQuarter / 8)
    #expect(profile.caesuraPauseTicks == MusicXMLTempoMap.ticksPerQuarter / 2)
    #expect(profile.ornamentSubdivisionTicks == MusicXMLTempoMap.ticksPerQuarter / 8)
    #expect(profile.unmeasuredTremoloSubdivisionTicks == MusicXMLTempoMap.ticksPerQuarter / 8)
