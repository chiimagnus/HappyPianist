...
@Test
func readerRejectsUnsafeRootfilePathBeforeExtraction() throws {
    let baseURL = FileManager.default.temporaryDirectory
        .appending(path: "MXLReaderSafetyTests")
        .appending(path: UUID().uuidString)
    try FileManager.default.createDirectory(at: baseURL, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: baseURL) }
    let metaInfURL = baseURL.appending(path: "META-INF")
    try FileManager.default.createDirectory(at: metaInfURL, withIntermediateDirectories: true)
    try Data("<container><rootfiles><rootfile full-path=\"../score.xml\"/></rootfiles></container>".utf8)
        .write(to: metaInfURL.appending(path: "container.xml"))
    try Data("<score-partwise/>".utf8).write(to: baseURL.appending(path: "score.xml"))
    ...
    #expect(throws: MXLReaderError.rejectedBySafetyPolicy(.archiveEntryNameIsUnsafe)) {
        _ = try MXLReader().readScoreXMLData(from: mxlURL)
    }
}

@Test
func readerRejectsFileBeforeOpeningAnOversizedArchive() throws {
    let fileURL = FileManager.default.temporaryDirectory.appending(path: "MXLReaderOversized-\(UUID().uuidString).mxl")
    defer { try? FileManager.default.removeItem(at: fileURL) }
    try Data([0, 1]).write(to: fileURL)

    #expect(throws: MXLReaderError.rejectedBySafetyPolicy(.sourceFileTooLarge)) {
