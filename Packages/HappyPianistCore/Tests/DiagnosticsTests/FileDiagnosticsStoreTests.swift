...
func diagnosticsStoreRetainsSevenCalendarDays() async throws {
    ...
    let store = FileDiagnosticsStore(paths: DiagnosticsPaths(rootDirectoryURL: root), calendar: calendar, now: { current })
    for offset in -8 ... 0 {
        try await store.append(testDiagnosticEvent(at: #require(calendar.date(byAdding: .day, value: offset, to: current))))
    }
