...
func diagnosticFileReferenceNormalizesSafeRelativePath() throws {
    let reference = try #require(DiagnosticFileReference(
...
func diagnosticEventTextRepresentationContainsStableFields() throws {
    let reference = try #require(DiagnosticFileReference(fileName: "example.musicxml", relativePath: "SongLibrary/scores/example.musicxml"))
...
func diagnosticEventCodableRoundTrips() throws {
    ...
    #expect(try JSONDecoder().decode(DiagnosticEvent.self, from: data) == event)
}
...
func diagnosticsDateTextBuildsStableTokensWithoutSharedFormatters() throws {
    ...
    let date = try #require(calendar.date(from: DateComponents(year: 2026, month: 7, day: 2, hour: 3, minute: 4, second: 5)))
...
func pianoPerformanceDiagnosticUsesOnlyLowCardinalityWhitelistedFields() {
    let event = PianoPerformanceDiagnosticSample(
        stage: .plan,
        outcome: .mismatch,
