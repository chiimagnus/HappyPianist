...
public enum PracticeMeasureIndexDiagnostic: Error, Equatable, Sendable {
    case passageBoundaryNotFound
    case passageOrderInvalid
    case noStepsInPassage
}

public struct PracticeMeasureIndex: Equatable, Sendable {
    public let steps: [PracticeStep]
    public let measureSpans: [MusicXMLMeasureSpan]

    public init(steps: [PracticeStep], measureSpans: [MusicXMLMeasureSpan]) {
        self.steps = steps
    ...
    public func occurrenceID(forStepIndex stepIndex: Int) -> PracticeMeasureOccurrenceID? {
    ...
    public func stepRange(forOccurrenceRange occurrenceRange: Range<Int>) throws -> Range<Int> {
    ...
    public func resolve(_ passage: PracticePassage) throws -> PracticeActiveRange {
