...
public struct MIDIPerformanceObservationAdapter {
    private var synchronizer: PerformanceClockSynchronizer
    private let estimatedLatencySeconds: Double?

    public init(estimatedLatencySeconds: Double? = nil) {
        synchronizer = PerformanceClockSynchronizer()
    ...
    public mutating func observation(
    ...
    public mutating func resetClockCalibration() {
        synchronizer.reset()
    }
