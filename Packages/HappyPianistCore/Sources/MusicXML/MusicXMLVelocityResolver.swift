...
public struct MusicXMLVelocityResolver {
    public struct WedgeSpan: Equatable {
        let start: MusicXMLWedgeEvent
        let stop: MusicXMLWedgeEvent
    }

    public let dynamicEvents: [MusicXMLDynamicEvent]
    public let wedgeEvents: [MusicXMLWedgeEvent]
    public let wedgeEnabled: Bool
    public let defaultVelocity: UInt8
    public let dynamicCurves: [MusicXMLDynamicCurve]
    public let wedgeApproximations: [MusicXMLWedgeApproximation]
    ...
    public func velocity(for note: MusicXMLNoteEvent) -> UInt8 {
        resolution(for: note).velocity
    }

    public func resolution(for note: MusicXMLNoteEvent) -> MusicXMLVelocityResolution {
