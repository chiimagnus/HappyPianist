...
class ChoiceQuestion(BaseModel):
    ...
    def validate_criteria(cls, value: dict[str, str | None]) -> dict[str, str | None]:
        if len(value) < 2:
            raise ValueError("choice questions require at least 2 criteria")
...
class ClassifierRequest(BaseModel):
    ...
    @classmethod
    def validate_questions(cls, value: dict[str, ChoiceQuestion]) -> dict[str, ChoiceQuestion]:
        if not value:
            raise ValueError("classifier requests require at least 1 question")
        if any(key.strip() == "" for key in value):
...
def _labels(count: int) -> list[str]:
    ...
    if count > len(labels):
        raise ValueError(f"choice question has too many criteria: {count}")
    return labels[:count]


def _shared_system(questions: dict[str, ChoiceQuestion]) -> str:
...
def _selected_question_block(instructions: str, detail: str) -> str:
    ...
    return (
        ...
        "options. Return only the requested JSON answer; do not explain or reason aloud.\n"
