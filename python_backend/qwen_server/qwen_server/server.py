...
class TransformersQwenRuntime:
    ...
    def load(self) -> None:
        with self._lock:
            ...
            self._model = model
            logger.info(
                "Qwen runtime loaded: model=%s device=%s",
                self._model_name_or_path,
                self._device,
            )
...
async def handle_classifier(request: web.Request) -> web.Response:
    ...
    try:
        ...
        logger.exception("Qwen classification failed")
        return web.json_response(
...
async def _load_runtime(app: web.Application) -> None:
    ...
    logger.info("Qwen runtime warm-up completed in %d ms", latency_ms)
...
async def _bonjour_start(app: web.Application) -> None:
    ...
    broadcaster = BonjourServiceBroadcaster(
        ...
        instance_name="HappyPianist Qwen Classifier",
        port=config.port,
        properties={
            b"path": b"/v1/classifier",
            b"protocol_version": b"1",
            b"engine": b"qwen-classifier",
...
def main(argv: list[str] | None = None) -> None:
    ...
    print(f"[qwen_server] model={config.model}", flush=True)
    print(f"[qwen_server] device={config.device}", flush=True)
    print(
        f"[qwen_server] listening=http://{config.host}:{config.port}/v1/classifier",
        flush=True,
    )
    web.run_app(
        create_app(config),
