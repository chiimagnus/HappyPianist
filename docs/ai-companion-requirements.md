...
- CompanionDecisionBackendProtocol 已经成为实时陪伴决策的统一协议；当前 RuleBasedCompanionDecisionBackend 根据按键、踏板、演奏速度、力度趋势和音符密度判断 AI 应该支持、稀疏、让位或静默；
...
2026-09-20 已在 Windows RTX 4060 8GB 上使用官方 Qwen3.5-0.8B 做单次前向决策实验。模型不生成解释文本，而是直接比较固定动作 A/B/C/D 的下一 token 概率。

- 模型加载到 GPU 后约占 1.46 GB 显存，测试峰值约 1.74 GB；
- 零样本输入约 190 token，热推理平均约 88 ms，但 5 个场景中只判断正确 1 个，几乎总是选择“正常陪奏”；
- 加入四个少样本示例后，输入约 500 token，热推理平均约 131 ms，5 个场景判断正确 3 个；
- 因此 0.8B 模型的本地实时性能已经足够，但通用 Qwen3.5 不能直接等同于 Jev 类决策模型。

当前不把这个实验模型接入正式产品。下一步如果继续这条路线，应优先验证 Qwen3.5-0.8B 的决策微调或增加专用决策读出头，而不是依赖越来越长的提示词。
...
这四条路线不是互相排斥的。比较合理的探索顺序是：确定性规则决策后端先建立基线；再测试本地 Jev 类 0.5B～1B 小模型；如果通用模型不够懂音乐，再训练专用音乐交互分类模型；等未来积累足够交互数据以后，再研究全双工音乐模型。

无论最终使用哪一种模型，底层确定性实时系统都必须存在。MIDI 连接、音符释放、延音踏板一致性、取消过期请求、播放调度、缓冲、网络超时和防止挂音等职责不能交给模型。

近期最值得做的低成本验证，是让 RuleBasedCompanionDecisionBackend 与一个本地 Jev 类小模型面对同一批钢琴场景，对比延迟、稳定性和决策质量。这个实验结果再决定是否需要训练专用音乐模型。
