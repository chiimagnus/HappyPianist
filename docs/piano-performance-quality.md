...
Bravura / SMuFL catalog 与 staff-space geometry 覆盖 whole/half/black heads、8th/16th/32nd/64th/128th flags、对应 rests、附点、升降/还原/重升降、谱号、调号、拍号及常用 marks；相邻二度、accidental/dot columns、ledger lines 与多声部同 tick 在 layout 层统一碰撞处理。
...
note value 读取 `ScoreNotationProjection.SourceNote.writtenRhythm` 与 written duration；staccato、release 与 fermata 改写的 `performedOffTick` 只用于演奏 occurrence，不再改变 written note value。whole 至 128th、附点、普通休止、`measure="yes"` 整小节休止与 tuplet source facts 均进入结构模型和测试。

剩余边界：比 128th 更短的音值、微分音及不在支持矩阵的组合保留 source identity、kind、reason 和节奏空间，不猜写为相邻受支持音值。

#### NOTATION-003：休止、属性、连梁与复调保持 source identity

**状态：常见双谱表支持矩阵已修复｜证据：代码确认 + geometry/layout/visual golden**

layout 生成真实 rests、barlines、clef/key/meter changes，并保留 voice、source stem、逐层 beam begin/continue/end/hooks、cross-staff group、ties、slurs 与 nested tuplets。整小节休止由 `<rest measure="yes">` 源事实决定，不要求虚构 `<type>`。source stem/beam 缺失时，才分别执行和弦中线/voice policy 与 meter/rest-boundary fallback；hand 不决定谱表或符干。renderer 还覆盖 repeat/ending、常用 articulation、fermata、arpeggio、fingering、dynamic、tempo 与 pedal marks；仅用于播放的 `<sound tempo>` / `<sound dynamics>` 不生成可见谱面标记。

解析器已经保留但渲染器尚未支持的 notehead、trill、mordent、turn、tremolo、glissando、breath 与 caesura 会进入携带 source ID、kind 和 reason 的中性 fallback 与聚合诊断，不静默改写为普通音头或直接丢弃。
