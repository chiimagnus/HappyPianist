...
- controller 只消费当前 guide 与已经标定的键盘坐标：不读 ARKit、不挂 `InputTargetComponent`/碰撞、不产生 observation。两手从 RealityKitContent 异步加载 Blender 生成的骨骼资产，渲染期只更新既有 21 关节与 root transform；reset 会取消资产加载和击键动作并移除所有子实体，不回退到旧基础体手型。
