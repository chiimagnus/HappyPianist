...
- 指法 ready 后，session 在同一 generation gate 下把纯值 snapshot 交给 off-main `PianoHandMotionClipBuilder`。builder 以拇指/其余手指各自的屈伸、首关节外展与帧间角速度为硬约束；不夹断不可能姿态，也不发布 coverage 不完整的单手 clip。每个被拒 occurrence 经同一 generation gate 交给现有 coverage，立刻回退键面贴片。`PianoHandMotionClip` 只包含键盘局部整手 transform、21 个 joint rotation、score/skeleton/generator metadata 与 occurrence coverage，结构上不允许 joint translation，故始终保留 authored bind-pose bone length；未完成或过期 clip 不发布。clip 的帧仍未进入 controller，T8 才由唯一播放器替换 P1 的逐帧 pose 路径。
