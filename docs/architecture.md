...
| 示范手引导 | `PianoDemonstrationHandsOverlayController`、`PianoGuideOverlayController` | 默认关闭的 AppStorage 偏好在沉浸空间二选一显示 Blender 生成的 21 关节骨骼双手或键面贴片；示范手按 guide 触发预备、落键、回弹和保持，只消费当前 guide 与键盘几何，不参与 ARKit、输入或进度，二维键盘始终保留高亮。 |
...
- 示范手偏好只保存开关；手型、触键目标、骨骼姿态、动画和键面贴片均是当前 guide 的派生渲染状态。资产加载失败只写统一诊断，关闭、scene suspend 与退出会取消加载/动作并移除示范手 root；Reducer、progress 与输入路径均不可引用它。
