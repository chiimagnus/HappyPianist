...
| 示范手引导 | `PianoDemonstrationHandsOverlayController`、`PianoHandMotionPlayer`、`PianoGuideOverlayController` | 默认关闭的 AppStorage 偏好在沉浸空间按 occurrence 混合 Blender 生成的 21 关节骨骼双手与键面贴片；Core 的 `PianoFingeringPlanner` 只消费 transport contact、校准后的纯值键盘快照和可调的 `MotionLimits`，在主 Actor 外以有界确定性 DP 生成指法或显式无解。session 随后把同一代 snapshot 交给 off-main `PianoHandMotionClipBuilder`；它验证指骨长度、手指/手掌 capsule、键面/按键碰撞、接触残差、屈伸与 joint/wrist 速度，任一手的 clip 失败即记录其 occurrence 并交回贴片。Core clip 只携带整手 transform、21 joint rotation 和 occurrence coverage，永不携带 RealityKit 或 joint translation，因此保持 authored bind-pose bone length。唯一 player 只按当前 audio transport 的 generation、位置、暂停和速率插值 clip；controller 只应用该 sample，缺 clip、资产或规划时继续显示贴片。它不参与 ARKit、输入或进度，二维键盘始终保留高亮。 |
