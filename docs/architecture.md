...
| 示范手引导 | `PianoDemonstrationHandsOverlayController`、`PianoGuideOverlayController` | 默认关闭的 AppStorage 偏好在沉浸空间按 occurrence 混合 Blender 生成的 21 关节骨骼双手与键面贴片；Core 的 `PianoFingeringPlanner` 只消费 transport contact、校准后的纯值键盘快照和可调的 `MotionLimits`，在主 Actor 外以有界确定性 DP 生成指法或显式无解。session 随后把同一代 snapshot 交给 off-main `PianoHandMotionClipBuilder`；它对拇指/其余手指分别施加屈伸、首关节外展、joint/wrist 速度与 wrist root transform 硬约束，任何手的 clip 失败即记录其 occurrence 并立即交回贴片。唯一的纯值 root planner 同时供尚未删除的 P1 pose/rig 路径使用，集中 authored MCP offset、手腕高度与镜像 rotation；T8 接管时删除旧路径而不迁移第二套几何。Core clip 只携带整手 transform、21 joint rotation 和 occurrence coverage，永不携带 RealityKit 或 joint translation，因此保持 authored bind-pose bone length。controller 只按 transport 的最大 pre-roll 选取下一 guide，旧 guide 的 stale attack 不会与下一 guide 合并。它不参与 ARKit、输入或进度，二维键盘始终保留高亮。 |
