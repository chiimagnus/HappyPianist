...
`make test` 会限制 Simulator boot（180 秒）、destination 查找（60 秒）和整次 action（900 秒），并开启单测试 timeout（默认 120 秒、最大 300 秒）。超时会终止独立进程组，并把不含 app container 的 Simulator 诊断写至 `.build/TestResults`。受控异步测试必须使用有界等待（如 `TestAsyncWait`），不得使用无界 `Task.yield()` 轮询；需要调整时只覆盖相应 Make 变量，不得移除边界。

`make build:mac` 和 `make test:mac` 只使用 macOS scheme/destination/result bundle，不启动或读取 visionOS Simulator。`build-for-testing`、语法检查或 Linux harness 都不能替代实际 `xcodebuild test`。
