...
仍留在 `Services/` 顶层的目录是跨业务或平台边界：`Library/` 负责曲库事务，`MusicXML/` 负责曲谱格式与准备管线，`Recording/` 负责 take 生命周期，`MIDI/`、`HandTracking/`、`PianoMode/RealPiano/`、`PianoMode/VirtualPiano/`、`Tracking/` 与 `Immersive/` 负责设备/空间适配。`Audio/AudioOutputVolumeSettings.swift` 是曲库试听和练习播放共用的音量设置，因此不归入单一业务模块；`Diagnostics/`、`Concurrency/`、`Time/` 与 `Utilities/` 是横切基础设施。
