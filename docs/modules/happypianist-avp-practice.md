...
手动“示范本节”按选定 step range 裁出同一条 plan timeline，保留 velocity、tempo、pedal、grace、arpeggio 与 fermata。当前音提示是独立的短 pitch preview，不是 reference playback。stop、seek、loop、播放错误、音频中断与 route change 都执行 reducer 生成的有序 reset commands，AVAudio/CoreMIDI 层只负责发送命令。
