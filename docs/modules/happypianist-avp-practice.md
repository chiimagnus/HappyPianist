...
曲库右侧 Ornament 直接编辑同一个 `PracticeRoundConfigurationController` 的 pending configuration。点击“去练习！”时：

- pending 未改变：保留已恢复的 step 与 paused 状态；
- pending 已改变：应用新配置并从所选片段起点开始；
- 不经过 confirmation dialog 或 setup sheet。

本轮开始后 active configuration 不可变。切换曲目时丢弃未开始的草稿；无历史进度使用整首、双手、100%、不循环。
...
`LibraryPracticePanelPresentation` 只从当前曲谱 identity、持久化小节事实和 pending configuration 派生：稳定小节数、恢复说明、卡点、小节地图与启动摘要。它不创建第二套进度状态，也不保存 UI presentation。

曲谱准备中的骨架页使用系统 placeholder redaction，不实现自定义 shimmer。准备失败通过 typed failure 提供可复制技术详情；取消旧曲目的 preparation 不视为失败。

历史趋势曲线属于后续需求。当前版本不新增日期序列或统计存储，也不展示伪造曲线。
