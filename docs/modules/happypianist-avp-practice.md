...
五线谱 viewport 中心 tick 由 session 即时派生：autoplay 使用连续播放位置，手动推进与暂停恢复使用当前 step，完成后停留在 active range 的最后一步。该 tick 不持久化；notation overlay 只负责高亮与范围裁剪，不负责定位。
...
手动“示范本节”按选定 step range 裁出同一条 plan timeline，保留 velocity、tempo、pedal、grace、arpeggio 与 fermata。当前音提示是独立的短 pitch preview，不是 reference playback。stop、seek、loop、播放错误、音频中断与 route change 都执行 reducer 生成的有序 reset commands，AVAudio/CoreMIDI 层只负责发送命令；`load`/`play` 只机械切换已显式 reset 的序列，不隐式追加第二组命令。AVAudio 会把会话、engine、sequence 与 render 失败发布为带恢复等级的 `PracticeAudioError`，reset 完成前不发布失败状态；interruption 仅在系统给出 `shouldResume` 后自动恢复，media-services reset 会重建整套音频对象。
...
输出指标按 scheduler generation 或 reset 操作低频汇总 scheduled/submitted/acknowledged、latency/jitter 分桶、late、dropped、cancelled 与 stuck-note prevention。平台没有发声回执时不伪造 acknowledged；汇总不含逐音 MIDI，可按现有七天策略导出。
