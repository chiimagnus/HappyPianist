...
composition root 是 `MacAppGraph`：它显式提供空的 bundled-library provider，因此首次启动一定从用户导入开始。曲库、端点设置与 MIDI-only practice 会在同一 host 内逐步接入共享 core；没有 AVP 的 `LiveAppGraph`、seed score、audio player 或 singleton 兼容路径。
